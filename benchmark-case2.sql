-- ============================================================
-- 케이스 2: 정류장 10개 (5명당 1개) → API 10번 호출
-- 예상 Before: ~13,430ms / After: ~1,343ms
-- ============================================================
...
SELECT
    ff.id,
    CONCAT('GGB21100', LPAD((ROW_NUMBER() OVER (ORDER BY u.id) - 1) DIV 5 + 1, 4, '0')),
    CONCAT('테스트정류장', (ROW_NUMBER() OVER (ORDER BY u.id) - 1) DIV 5 + 1),
    '21000',
    37.5665,
    126.9780,
    'GGB210000006',
...
SELECT COUNT(DISTINCT station_id) AS unique_stations FROM favorites; -- 기대값: 10
